//
//  InviteVisitorsViewController.swift
//  RSOWorkspace
//
...
protocol sendVisitorEmailDelegate:AnyObject{
    func sendVisitorEmail(email:String)
}
class InviteVisitorsViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        txtSearch.placeholderText = "Search  Visitors"
    ...
    @IBAction func btnAddVisitorTappedAction(_ sender: Any) {
        if let txtguestEmail = txtSearch.text{
            visitorEmail = txtguestEmail
        }
        visitorEmailDelegate?.sendVisitorEmail(email: visitorEmail)
        dismiss(animated: true)
    }
